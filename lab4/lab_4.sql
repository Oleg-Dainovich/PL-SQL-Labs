CREATE OR REPLACE FUNCTION parse_json(
    ...
    v_join_conditions   VARCHAR2(8000);
   	v_join				VARCHAR2(8000);
   	v_join_type		    VARCHAR2(8000);
   	v_join_table	    VARCHAR2(8000);
    v_join_col		    VARCHAR2(8000);
    v_join_col_2		VARCHAR2(8000);
...
BEGIN
    ...
    IF v_query_type = 'SELECT' THEN
        ...
        v_json_array := v_json_data.get_array('join');
       	FOR i IN 0..v_json_array.get_size() - 1 LOOP
	       	v_temp_obj := TREAT(v_json_array.get(i) AS json_object_t);
	        v_join_type := v_temp_obj.get_string('join_type');
	   		v_join_table := v_temp_obj.get_string('table');
	        v_join_col := v_temp_obj.get_string('col');
	        v_join_col_2 := v_temp_obj.get_string('col2'); 
	        v_join := v_join_type || ' JOIN ' || v_join_table || ' ON ' || v_tables || '.' || v_join_col || ' = ' || v_join_table || '.' || v_join_col_2 || ' ';         
		END LOOP;
    ...
    ELSIF v_query_type = 'CREATE TABLE' THEN
        ...
        FROM dual;
       	...
       	v_sql := '';
        v_sql := 'CREATE SEQUENCE ' || v_tables || '_seq START WITH 1';
        DBMS_OUTPUT.PUT_LINE(v_sql);
        EXECUTE IMMEDIATE v_sql;
       
        v_sql := '';
        v_sql := '
CREATE OR REPLACE TRIGGER trigger_' || v_tables || '_pk_autoincrement
BEFORE INSERT ON ' || v_tables || ' FOR EACH ROW
BEGIN
  ...
  "filter_conditions": [
    {
      ...
      "condition": {
        ...
        "filter_conditions": [
          {
            "condition": "t2.val like ''%a%''",
            "operator": "AND"
          },
          {
            "condition": "t2.num between 3 and 5",
            "operator": "AND"
          }
        ],
        "operator": "IN",
...
DECLARE
  ...
  json_data CLOB := '{
      "query_type": "UPDATE",
      "table": "tb_1",
      "set": [ {"field": "id", "value": "100"}, {"field": "name", "value": "''AAAAAA''"}],
...
DECLARE
  ...
  json_data CLOB := '{
      ...
      "primary_keys": ["id"],
      "foreign_keys": [{"field": "val", "table": "tb_1", "ref_field": "id"}]
   }';
